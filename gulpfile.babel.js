...
const watchedBrowserify = watchify( //#1 수정사항 지켜보는 함수
  browserify({ //#0 typescript파일에서 require 또는 import로 연결해 준다. 
    basedir:'.',
    debug:true,//debug:true의 값을 사용하면, tsify는 번들된 JavaScript 파일 안에 소스맵을 내보낸다. 소스맵을 사용하면 번들로 제공된 JavaScript 대신 브라우저에서 원본 Typescript 코드를 디버깅할 수 있다.  크롬 browser 디버깅 탭을 열고 디버깅파일.ts 안에 브레이크 포인트를 넣으면 소스 맵이 작동하는지 테스트할 수 있다. 페이지를 새로고침하면 브레이크 포인트가 페이지를 일시 중지하고 디버깅파일.ts 에서 import로 먼저 들어오는디버깅파일.ts 상단의 @import 파일명.ts를 디버깅할 수 있어야 한다. 
    entries:['src/ts/common.ts'],
    cache:{},
    packageCache:{}
  })
...
const webServer = async () => {
  try {
    const port = await portfinder.getPortPromise({ port: 8000 }); // 8000부터 사용 가능한 포트 찾기
    gulp.src("build").pipe(
      gws({
        livereload: true,
        open: true,
